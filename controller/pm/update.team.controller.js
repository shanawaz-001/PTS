    ...
    try {
        ...
        if(team){
            ...
            .exec((e,d)=>{
                ...
                return res.status(200).send({type: 'success', message: 'Team updated/added to the Project'})
            })
        }else{
            await Team.findOneAndUpdate({projectRef: projectRef},{teamLeader:teamLeader, teamMembers: teamMembers},{new:true,upsert:true})
            .exec((e,d)=>{
                if(e) return res.status(400).send({type: 'error', message: e.message})
                return res.status(200).send({type: 'success', message: 'Team updated/added to the Project'})
            })
